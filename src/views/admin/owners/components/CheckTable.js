...
export default function CheckTable(props) {
  ...
  }, [deleteModel, props.isOpen,edit,selectedId])

 

  return (
    <>
      <Edit isOpen={edit} size={size} onClose={handleCloseEdit} id={data[selectedId]}/>
    <Card
      direction="column"
      w="100%"
      px="0px"
    ...
    >
      ...
      <Flex px="25px" justify="space-between" mb="20px" align="center">
        <Text
          ...
          lineHeight="100%"
        >
          Owners (<CountUpComponent targetNumber={data?.length} />)
        </Text>
        {/* <Menu /> */}
                ...
                return (
                  <Tr {...row?.getRowProps()} key={i}>
                    {row?.cells?.map((cell, index) => {
                      ...
                      if (cell?.column.Header === "#") {
                        data = (
                          <Flex align="center">
                            ...
                            <Link to={user?.role !== 'admin' ? `/ownerView/${cell?.row?.original._id}` : `/admin/ownerView/${cell?.row?.original._id}`}>
                      ...
                      } else if (cell?.column.Header === "Passport Number") {
                        data = (
                          <Text color={textColor} fontSize="sm" fontWeight="700">
                            {cell?.value}
                          </Text>
                        );
                      } else if (cell?.column.Header === "Emirates ID") {
                        data = (
                          <Text color={textColor} fontSize="sm" fontWeight="700">
                            {cell?.value}
                          </Text>
                      ...
                      }else if (cell?.column.Header === "Bank Account No") {
                        data = (
                          <Text color={textColor} fontSize="sm" fontWeight="700">
                            {cell?.value}
                          </Text>
