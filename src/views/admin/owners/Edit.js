...
const Edit = (props) => {
  ...
  const EditData = async () => {
    ...
    try {
      ...
      let response = await putApi(
        `api/owner/edit/${param.id? param.id : props.id._id}`,
        values
      );
  ...
  let response = typeof props.id == "object" ? props?.id : null;
  ...
  const fetchData = async () => {
    try {
      ...
      response = await getApi(
        "api/owner/view/",
        param.id ? param.id : props.id
      );
      console.log(response);
        response = response?.data?.owner;
      ...
      values.dob = moment(response?.dob).format("YYYY-MM-DD");

      values.type = response?.type;
      values.name = response?.name;
      values.email = response?.email;
      values.mobileNo = response?.mobileNo;
      values.address = response?.address;
      values.passportNo = response?.passportNo;
      values.emiratesId = response?.emiratesId;
      values.bankName = response?.bankName;
      values.bankAccountNo = response?.bankAccountNo;
      values.createBy = response?.createBy;
    } catch (e) {
      console.log(e);
    } finally {
      setIsLoding(false);
  ...
  console.log("this is the eidt", values,props);
  return (
    <div>
            ...
            <Grid templateColumns="repeat(12, 1fr)" gap={3}>
              ...
              <GridItem colSpan={{ base: 12 }}>
                ...
                <Input
                  ...
                  mb={errors.address && touched.address ? undefined : "10px"}
                  fontWeight="500"
                  borderColor={
