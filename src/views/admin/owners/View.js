...
import AddEmailHistory from "../emailHistory/components/AddEmail.js";
import AddMeeting from "../meeting/components/Addmeeting.js";
import MeetingTable from "../meeting/components/CheckTable.js";
import AddPhoneCall from "../phoneCall/components/AddPhoneCall.js";
import TaskTable from "../task/components/CheckTable.js";
import AddTask from "../task/components/addTask.js";
import Add from "./Add.js";
import Delete from "./Delete.js";
import Edit from "./Edit.js";
import ColumnsTable from "./components/ColumnsTable.js";
import PhoneCall from "./components/phonCall.js";
...
const View = () => {
    ...
    const fetchData = async () => {
        setIsLoding(true)
        let response = await getApi('api/owner/view/', param.id)
    ...
    return (
        <>
            ...
            <Delete isOpen={deleteModel} onClose={setDelete} method='one' url='api/owner/delete/' id={param.id} />
                                            ...
                                            <Grid templateColumns={{ base: "1fr" }} gap={4}>
                                                ...
                                                <Grid templateColumns={{ base: "1fr" }} gap={4}>
                                                    ...
                                                    <GridItem colSpan={{ base: 2, md: 1 }}>
                                                        ...
                                                        <Text>{data?.mobileNo ? data?.mobileNo : 'N/A'}</Text>
                                                    </GridItem>
                                                    <GridItem colSpan={{ base: 2 }}>
                                                        <Text color={'blackAlpha.900'} fontSize="sm" fontWeight="bold">
                                                            Email Address
                                                        </Text>
                                                        <Text onClick={() => setAddEmailHistory(true)} color='green.400' sx={{ cursor: 'pointer', '&:hover': { color: 'blue.500', textDecoration: 'underline' } }}>{data?.email ? data?.email : 'N/A'}</Text>
                                                    </GridItem>
                                                    <GridItem colSpan={{ base: 2 }}>
                                                        <Text color={'blackAlpha.900'} fontSize="sm" fontWeight="bold">
                                                             Address
                                                        </Text>
                                                        <Text>{data?.address ? data?.address : 'N/A'}</Text>
                                                    </GridItem>
                                                    <GridItem colSpan={{ base: 2 }}>
                                                        <Text color={'blackAlpha.900'} fontSize="sm" fontWeight="bold">
                                                            Type
                                                        </Text>
                                                        <Text>{data?.type ? data?.type : 'N/A'}</Text>
                                                    </GridItem>
